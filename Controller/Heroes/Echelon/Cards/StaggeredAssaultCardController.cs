...
namespace Cauldron.Echelon
{
    public class StaggeredAssaultCardController : TacticBaseCardController
    {
        ...
        {
            //"Once per turn when a hero target deals an instance of 2 or more damage, {Echelon} may deal 1 target 1 melee damage."
            AddTrigger((DealDamageAction dd) => dd.DamageSource.IsTarget && dd.DamageSource.IsHero && dd.Amount >= 2 && !HasBeenSetToTrueThisTurn(damageKey), DealFollowupDamageResponse, TriggerType.DealDamage, TriggerTiming.After, isActionOptional: true);
        }
