...
namespace Cauldron.Starlight
{
    public class WarpHaloCardController : StarlightCardController
    {
        public WarpHaloCardController(Card card, TurnTakerController turnTakerController) : base(card, turnTakerController)
        {
            ...
            SpecialStringMaker.ShowListOfCards(new LinqCardCriteria((Card c) => base.IsVillainTarget(c) && IsNextToConstellation(c), useCardsSuffix: false, singular: "villain target next to a constellation", plural: "villain targets next to a constellation"));
        }
