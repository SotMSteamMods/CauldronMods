...
namespace Cauldron.SuperstormAkela
{
    public class RideTheCurrentsCardController : SuperstormAkelaCardController
    {
        ...
        public override IEnumerator Play()
        {
            ...
            IEnumerator coroutine = GameController.DetermineTurnTakersWithMostOrFewest(
                false,
                1,
                1,
                (TurnTaker tt) => (!tt.IsIncapacitatedOrOutOfGame && tt.BattleZone == base.BattleZone),
                tt => GameController.FindCardsWhere((Card c) => c.IsInPlay && !c.IsCharacter && c.Owner == tt && GameController.IsCardVisibleToCardSource(c, GetCardSource())).Count(),
                SelectionType.PlayTopCard,
                storedResults,
                cardSource: GetCardSource(),
                battleZone: base.BattleZone);
            if (base.UseUnityCoroutines)
            {
                yield return base.GameController.StartCoroutine(coroutine);
            }
