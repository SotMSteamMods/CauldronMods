...
namespace Cauldron.DungeonsOfTerror
{
    public class DubiousEdiblesCardController : DungeonsOfTerrorUtilityCardController
    {
        ...
        public override IEnumerator Play()
        {
            ...
            IEnumerator coroutine = CheckForNumberOfFates(TurnTaker.Trash.TopCard.ToEnumerable(), storedResults);
            ...
            if (storedResults.Any() && storedResults.First() == 1)
            {
                ...
                message = GameController.SendMessageAction($"The top card of the environment trash is a fate card!", Priority.High, GetCardSource(), showCardSource: true);
            ...
            else if(storedResults.Any() && storedResults.First() == 0)
            {
                ...
                message = GameController.SendMessageAction($"The top card of the environment trash is not a fate card!", Priority.High, GetCardSource(), showCardSource: true);
