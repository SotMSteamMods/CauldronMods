...
namespace Cauldron.Menagerie
{
    public class TakIshmaelCardController : MenagerieCardController
    {
        ...
        private IEnumerator PlayTopCardAndDealDamageResponse(PhaseChangeAction action)
        {
            ...
            coroutine = base.DealDamageToHighestHP(base.Card, 1, (Card c) => c.IsHero, (Card c) => base.FindCardsWhere(new LinqCardCriteria((Card spec) => base.IsSpecimen(spec) && spec.IsInPlayAndHasGameText)).Count(), DamageType.Projectile);
            if (base.UseUnityCoroutines)
            {
                yield return base.GameController.StartCoroutine(coroutine);
            }
