...
namespace Cauldron.Impact
{
    public class MassDriverCardController : CardController
    {
        ...
        public override IEnumerator Play()
        {
            ...
            coroutine = GameController.SelectTargetsAndDealDamage(DecisionMaker, new DamageSource(GameController, this.CharacterCard), target => this.TurnTaker.GetCardsWhere((Card c) => c.IsInPlayAndHasGameText && c.IsOngoing).Count(), DamageType.Projectile, () => 1, false, 1, cardSource: GetCardSource());
            if (UseUnityCoroutines)
            {
                yield return GameController.StartCoroutine(coroutine);
            }
