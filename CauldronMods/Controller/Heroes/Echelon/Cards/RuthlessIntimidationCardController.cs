...
namespace Cauldron.Echelon
{
    public class RuthlessIntimidationCardController : TacticBaseCardController
    {
        ...
        {
            //Increase damage dealt to the non-hero target with the lowest HP by 1.
            AddTrigger((DealDamageAction dd) => !IsHeroTarget(dd.Target) && CanCardBeConsideredLowestHitPoints(dd.Target, (Card c) => !IsHeroTarget(c)), MaybeIncreaseDamageResponse, TriggerType.IncreaseDamage, TriggerTiming.Before);
        }
        ...
        private IEnumerator MaybeIncreaseDamageResponse(DealDamageAction dd)
        {
            if (GameController.PretendMode)
            {
                ...
                IEnumerator coroutine = DetermineIfGivenCardIsTargetWithLowestOrHighestHitPoints(dd.Target, highest: false, (Card card) => !IsHeroTarget(card), dd, storedResults);
